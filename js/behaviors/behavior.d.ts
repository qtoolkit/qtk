import { Widget } from "../controls/widget";
import Events = require("../events");
/**
 * Behavior代表控件的一种行为特性，比如Resizable/Movable/Draggable/Droppable等。
 * 把这些行为特性抽象出来单独实现，一方面可以避免让Widget变得太复杂，另一方面可以最大限度的重用这些行为特性。
 *
 * 任何一个Widget，都可以使用useBehavior来启用某种Behavior。比如下面的代码让image具有Resizable的特性:
 *
 * ```
 * image.useBehavior("resizable", {all:true});
 * ```
 */
export declare class Behavior {
    /**
     * Behavior的类型名。
     */
    type: string;
    /**
     * Behavior作用的Widget。
     */
    widget: Widget;
    /**
     * 构造函数。主要是注册一些事件，把这些事件映射在成员函数上，子类只需要重载这些成员函数即可。
     * @param type 类型名。
     * @param widget Behavior作用的Widget。
     * @param options 初始化参数，与具体的Behavior有关。
     */
    constructor(type: string, widget: Widget, options: any);
    /**
     * 初始化。在具体的Behavior的实现中，可以重载此函数做些初始化的工作。
     * @param options 初始化参数，与具体的Behavior有关。
     */
    protected init(options: any): any;
    setOptions(options: any): any;
    /**
     * 析构函数。 主要是注销事件的处理函数。
     */
    protected dispose(): void;
    private _json;
    toJson(): any;
    /**
     * 子类重载此函数，可以处理Widget的按键按下事件。
     */
    protected onKeyDown(evt: Events.KeyEvent): void;
    /**
     * 子类重载此函数，可以处理Widget的按键抬起事件。
     */
    protected onKeyUp(evt: Events.KeyEvent): void;
    /**
     * 子类重载此函数，可以处理全局的按键按下事件。
     */
    protected onKeyDownGlobal(evt: CustomEvent): void;
    /**
     * 子类重载此函数，可以处理全局的按键抬起事件。
     */
    protected onKeyUpGlobal(evt: CustomEvent): void;
    /**
     * 子类重载此函数，可以处理Widget的PointerEnter事件。
     */
    protected onPointerEnter(evt: Events.PointerEvent): void;
    /**
     * 子类重载此函数，可以处理Widget的PointerLeave事件。
     */
    protected onPointerLeave(evt: Events.PointerEvent): void;
    /**
     * 子类重载此函数，可以处理Widget的PointerDown事件。
     */
    protected onPointerDown(evt: Events.PointerEvent): void;
    /**
     * 子类重载此函数，可以处理Widget的PointerMove事件。
     */
    protected onPointerMove(evt: Events.PointerEvent): void;
    /**
     * 子类重载此函数，可以处理Widget的PointerUp事件。
     */
    protected onPointerUp(evt: Events.PointerEvent): void;
    private pointerEnterFunc;
    private pointerLeaveFunc;
    private pointerDownFunc;
    private pointerMoveFunc;
    private pointerUpFunc;
    private keyDownFunc;
    private keyUpFunc;
    private keyDownGlobalFunc;
    private keyUpGlobalFunc;
}
/**
 * Behavior工厂类。
 *
 * 具体的Behavior需要调用BehaviorFactory.register注册自己，useBehavior才能找到对应的Behavior。
 */
export declare class BehaviorFactory {
    private static creators;
    /**
     * 注册Behavior
     * @param type Behavior的类型名。
     * @param creator Behavior创建函数。
     */
    static register(type: string, creator: Function): void;
    /**
     * 创建Behavior。目前只在useBehavior中会用到。
     * @param type Behavior的类型名。
     * @param widget Behavior作用的Widget。
     * @param options Behavior的初始化参数。
     */
    static create(type: string, widget: Widget, options: any): Behavior;
}
